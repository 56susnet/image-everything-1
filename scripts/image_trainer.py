...
def get_model_path_all(path: str) -> str:
    if os.path.isdir(path):
        files = [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]
        if len(files) == 1 and files[0].endswith(".safetensors"):
            return os.path.join(path, files[0])
    return path
def merge_model_config_todo(default_config: dict, model_config: dict) -> dict:
    merged = {}
...
def count_images_in_directory_job(directory_path: str) -> int:
...
def get_config_for_model_job(lrs_config: dict, model_name: str) -> dict:
...
def load_lrs_configs(model_type: str, is_style: bool) -> dict:
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def create_configs(task_id, model_path, model_name, model_type, expected_repo_name, trigger_word: str | None = None):
...
def run_training_for_all(model_type, config_path):
